...
import { ArrowRight, Scan, Shield, Zap, Eye, Lock, Users } from 'lucide-react';
    ...
    <div className="bg-white">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid lg:grid-cols-2 gap-12 items-center relative z-10 py-20">
          <motion.div initial={{ opacity: 0, x: -50 }} animate={heroInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.8 }} className="bg-white rounded-2xl p-8 shadow-xl">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-ayonix-teal/20 rounded-full border border-ayonix-teal/30 mb-6">
              <Scan className="w-4 h-4 text-ayonix-teal" />
              <span className="text-ayonix-teal text-sm font-medium">Use Case</span>
            </div>
            
            <h1 className="text-4xl md:text-6xl font-bold text-gray-900 mb-6">Face Recognition</h1>
            <p className="text-xl text-gray-700 mb-8">
              Advanced biometric identification for security and access control. State-of-the-art face recognition technology with unmatched accuracy even in challenging conditions.
            </p>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <Button asChild size="lg" variant="outline" className="border-2 border-ayonix-teal text-ayonix-teal hover:bg-ayonix-teal hover:text-white rounded-full">
                <Link href="http://va.ayonix.com" target="_blank">Open Demo</Link>
              </Button>
            </div>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.9 }} animate={heroInView ? { opacity: 1, scale: 1 } : {}} transition={{ duration: 0.8, delay: 0.3 }} className="relative h-80 lg:h-96 rounded-2xl overflow-hidden shadow-2xl">
            <Image src="/images/face-detection.jpg" alt="Face Recognition" fill className="object-cover" />
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[
              { icon: Scan, title: 'Real-Time Detection', description: 'Instant face detection and recognition' },
              { icon: Eye, title: 'Liveness Detection', description: 'Prevent spoofing with anti-fraud measures' },
              { icon: Users, title: 'Multi-Face Tracking', description: 'Track multiple faces simultaneously' },
              { icon: Shield, title: '99.9% Accuracy', description: 'Industry-leading recognition rates' },
              { icon: Zap, title: 'Edge Processing', description: 'Run on local devices for privacy' },
              { icon: Lock, title: 'Access Control', description: 'Secure entry management systems' },
            ].map((feature, i) => (
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid md:grid-cols-4 gap-8">
            {[
              ...
              { value: '10M+', label: 'Faces Matched/Second' },
      ...
      <section className="py-20">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-6">Secure Your Premises with AI</h2>
